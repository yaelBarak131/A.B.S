...
@WebServlet(name = "=GetLoanForSellServlet", urlPatterns = {"/getLoanForSellServlet"})
public class GetLoanForSellServlet extends HttpServlet {
            ...
            try (PrintWriter out = response.getWriter()) {
                ...
                synchronized (getServletContext()) {
                    ...
                    if (rewindModeFromParameter.equals(OFF))
                        customers = engine.findLoansCustomerCanSell(usernameFromParameter);
                    else if (rewindModeFromParameter.equals(ON)) {
                        customers = engine.getLoanToSellInRewindMode(usernameFromParameter, Integer.parseInt(currYazFromParameter));
                    }
